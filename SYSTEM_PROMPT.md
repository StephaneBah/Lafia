...
5. **One application per actor.** `web/citoyen`, `web/soin`, `web/caisse`, `web/pharmacie` are separate Next.js apps, each on its own subdomain. An app calls **its own service plus `identite`**, through the gateway, and nothing else. Apps are not on the noyau's network, never speak FHIR and store no medical data. They share the design system `web/design/`, a workspace package built in, never a runtime dependency. A new actor (laboratoire, télémédecine, a third party) joins the same way: its own app, its own service, FHIR to the noyau. See `docs/adr/0001-une-application-par-acteur.md`.
