...
**À fournir :** `value`/`onChange` (reçoit les chiffres seuls) ou `defaultValue`, `onComplete(npi)` appelé une fois au 13e chiffre (l'application soin fait alors glisser la carte patient), `label` (« NPI » par défaut), `hint`, `error`, `size`, `autoFocus` sur l'écran de recherche.
