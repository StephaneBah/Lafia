...
Today the stack runs ten containers, defined in `docker-compose.yml`. Three actors, soin, caisse and pharmacie, each have one application and one service:
...
| `application-soin`, `application-caisse`, `application-pharmacie` | Node.js, Next.js | The **application** of one actor (soignants, caissiers, pharmaciens): the pages served on `soin.<domaine>`, `caisse.<domaine>`, `pharmacie.<domaine>`. Its server asks its own service for data, through the gateway. It stores no medical data and never speaks FHIR. | none |
| `soin`, `caisse`, `pharmacie` | Python, FastAPI | The **service** of one actor: care, cashier, pharmacy. Each exposes its own API under `/api/<service>`, and reads and writes medical data by speaking FHIR to the noyau. | none |
...
`web/` is not a container either. It is an npm workspace holding the applications (`web/soin/`, `web/caisse/`, `web/pharmacie/`) and two packages they share. `web/commun/` is to the applications what `commun/` is to the services: how an application reaches its service through the gateway, and the page every application shows today. `web/design/` is the design system. Both are compiled into each application when its image is built, the way `commun/` is copied into each service: nothing of them is loaded at runtime. The design system is empty until F0 fills it; today it only gives the page a legible base stylesheet.

Still to come: the `citoyen` service and its application.
...
`/sante` checks the whole chain, and each broken link gives a different answer. caisse and pharmacie answer the same way under their own prefix, since their `/sante` comes from the same place:
...
3. **The service decides whether it serves that role.** soin serves médecin and infirmier, caisse serves caissier, pharmacie serves pharmacien. Each rule is in the service's own `regles/acces.py`, for instance `services/soin/src/soin/regles/acces.py`. `/api/caisse/session` and `/api/pharmacie/session` behave like soin's, each with its own roles.
...
| `403` | A valid token of a role the service does not serve: for soin, caissier, pharmacien, citoyen. |
...
Later, the `citoyen` service and its application join the same way.
...
| `application-soin`, `application-caisse`, `application-pharmacie` | Its own service and `identite`, through the gateway | `noyau`, by name or by address: the application is only on the `passerelle` network. Another actor's service: the gateway answers 404 to `/api/soin/*` on `caisse.<domaine>`, and so on. |
| `soin`, `caisse`, `pharmacie` | `noyau`, over FHIR | `base-noyau`'s data: only HAPI holds the database password. |
...
`--wait` returns when every container is healthy. Each container has `restart: unless-stopped`, so it comes back after a crash or a reboot. Each also has a memory limit, so the whole stack fits on one virtual machine: HAPI gets 2 GB with a 1 GB Java heap, PostgreSQL 512 MB, the gateway 128 MB, each service and each application 256 MB, about 4.4 GB in all.
...
|---|---|---|
| `LAFIA_DOMAINE` | Caddy: every application is served on `<application>.<domaine>`. soin, caisse, pharmacie: the development key is allowed on `localhost` only | `localhost` |
| `NOYAU_BASE_MOT_DE_PASSE` | PostgreSQL and HAPI | `noyau-local` |
| `JETON_CLE_PUBLIQUE` | soin, caisse, pharmacie, to verify tokens: the base64 line of an Ed25519 public key PEM | Empty: the development key, on `localhost` only |
...
- `tests/test_acteurs.py` runs the same checks for every actor, from one table: each actor's title and the roles its service serves. Session tests take every admitted role, every refused role and every invalid token, each by cookie and by `Authorization` header; the cross-actor test asks each subdomain for every other actor's `/sante` and expects 404.
...
| `services/<service>/src/<service>/regles/acces.py` | The roles a service serves |
...
- **The service of an actor** is `services/<acteur>/`: its routes, handed to `creer_service()` from `commun`, and its `regles/`, which name the roles it serves and are enforced with `VerificateurDeJetons`. `services/Dockerfile` builds it with `commun`; if it speaks FHIR, its Compose entry merges `x-service-fhir`, which puts it on both networks and gives it `NOYAU_URL`, `JETON_CLE_PUBLIQUE` and `LAFIA_DOMAINE`.
- **A new actor** (citoyen, later a laboratoire) means one service, one application, and one line in the `Caddyfile`, `import acteur <nom>`, which routes `/api/<nom>/*` to its service, `/api/identite/*` to identite, and nothing else under `/api/`. The test suite gains one row in the actor table of `tests/test_acteurs.py`, its subdomain in `tests/test_identite.py`, and its application in the isolation probes of `tests/test_isolement.py`. Nothing existing changes.
